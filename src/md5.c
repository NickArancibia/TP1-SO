...
void * createSharedMemory(char* name,int size, int* fd) {
    *fd = shm_open(name, O_CREAT | O_RDWR, 0666);
    if (*fd == -1) {
        fprintf(STDERR_FILENO,"failed shm_open");
        exit(EXIT_FAILURE);
    }

    if (ftruncate(*fd, size) == -1) {
        fprintf(STDERR_FILENO,"failed ftruncate");
        close(*fd);
        shm_unlink(SHM_NAME);
        exit(EXIT_FAILURE);
    }
    ...
    if(shm_ptrBase == MAP_FAILED) {
        fprintf(STDERR_FILENO,"failed mmap");
        close(*fd);
        shm_unlink(SHM_NAME);
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char const *argv[])
{
    ...
    *((int *)(shm_ptrBase)) = filesQty;
    ...
    if (sem_init(semAddress, 1, 0) == -1) {
        write(STDERR_FILENO,"failed sem_init",20);
        ...
        for(int j= 1,i=0; i < 4 ;i++,j+=2) {
            if(FD_ISSET(pipesFd[i*2+1][0],&read_fds)) {
                ...
                *spacePos = '\0'; 
                ...
                strncpy(output[idxOut].md5,entryPipe,strlen(entryPipe));
